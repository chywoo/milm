# MILM 향후 개선 및 발전 과제 (Roadmap & Recommendations)
...
- **현황**: `src/infer.py`에서 토큰을 하나 생성할 때마다 전체 누적 시퀀스 `input_ids[:, -seq_len:]`를 모델에 다시 입력하여 매 스텝마다 중복 $Q, K, V$를 재연산 ($\mathcal{O}(N^2)$).
- **개선안**:
