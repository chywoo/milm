...
- 작업을 검증 가능한 목표로 구체화하십시오:
  - "유효성 검사 추가" $\rightarrow$ "잘못된 입력에 대한 테스트 작성 후 통과시키기"
  - "버그 수정" $\rightarrow$ "버그를 재현하는 테스트 작성 후 통과시키기"
  - "X 리팩토링" $\rightarrow$ "리팩토링 전후 테스트가 동일하게 통과하는지 확인하기"
...
- **승인된 실행 환경**:
  - 프로젝트 루트(`/home/chywoo/workspace/milm`) 및 활성화된 Python 가상환경
- **임시 스크립트 및 데이터 작업 경로**:
...
### 2. 설정 검증 (Configuration Check)
- **YAML 설정 로드 테스트**:
  ```bash
  python -c "from config import load_config; m, t = load_config('config.yaml'); print('ModelConfig:', m); print('TrainConfig:', t)"
  ```
- **설정 템플릿 복사**:
  ```bash
...
### 3. 정적 검사 및 코드 스타일 (Static Checks & Lint)
- **코드 포맷팅**:
  ```bash
  black .
  ```
...
- **타입 검사**:
  ```bash
  mypy config.py model.py dataset.py train.py infer.py evaluate.py
  ```

### 4. GPU 프로파일링 (Profiling Commands)
- **Nsight Systems 학습 프로파일링**:
  ```bash
  nsys profile -t cuda,nvtx,osrt -s cpu --output=profile_train python train.py
  ```
- **Nsight Compute 커널 분석 (FlashAttention SDPA)**:
  ```bash
  ncu --nvtx --nvtx-include "FlashAttention_SDPA" --set full -o profile_sdpa_kernel python train.py
  ```

### 📌 명령어 사용 규칙 (Command Rules)
- 변경 사항을 검증할 때는 가장 빠르고 범위가 좁은 검사부터 실행하십시오.
...
```text
milm/
├── config.py             # ModelConfig/TrainConfig 및 config.yaml 파서 로직
├── config.yaml           # 로컬 모델 및 학습 하이퍼파라미터 설정 파일
├── config.yaml.template  # 설정 파일 템플릿 (버전 관리 대상)
├── model.py              # 트랜스포머(Decoder-Only MILM) 모델 및 서브모듈 아키텍처
├── dataset.py            # CharTokenizer 및 Next-Token Prediction 시퀀스 데이터셋
├── train.py              # AMP, 스케줄러, 최적화 및 모델 체크포인트 저장 학습 루프
├── infer.py              # Top-k/Top-p/온도 조절/반복 페널티 기반 자동회귀 텍스트 생성 추론 엔진
├── evaluate.py           # PPL, BLEU-4, ROUGE-L, 벤치마크 스위트 정량/정성 평가 모듈
├── README.md             # 프로젝트 개요, 아키텍처 다이어그램 및 주요 사용법
├── ROADMAP.md            # 단계별 성능 개선 및 최신 아키텍처/최적화 로드맵
├── TRAINING_REPORT.md    # 모델 학습 결과, 손실 함수 추이 및 과적합 분석 보고서
├── checkpoints/          # 최적 모델 가중치(best_model.pt) 및 토크나이저(tokenizer.json)
├── data/                 # 학습용 원본 코퍼스 (.txt)
└── llm_profile.nsys-rep  # NVIDIA Nsight Systems 프로파일링 리포트 아티팩트
```

### 📌 구조 규칙 (Structure Rules)
- **계층 분리**: 각 모듈의 고유 역할에 맞는 코드만 해당 파일에 작성합니다.
- **모듈 간 의존성 방향 준수**: 하위 기반 모듈(`config.py`, `model.py`, `dataset.py`)이 상위 실행 파이프라인(`train.py`, `infer.py`, `evaluate.py`)을 참조하지 않도록 합니다.
- **새로운 파일 생성 시**: 유사한 성격의 기존 파일 인근에 위치시키고 `README.md`의 구조도에 반영합니다.

---

## 🏛️ 아키텍처 경계 및 원칙 (Architecture Boundaries)
...
4. **`train.py` / `infer.py` / `evaluate.py`**:
